...
class ControllerDivision extends Controller
{
    ...
    public function liste_demandes_n(Request $request)
    {
        ...
        $jointure1 = DB::table('users')
            ...
            ->select('demandes.*', 'users.*')
            ->get();
        $dmd_n_lu = (demande::where('status_dmd', '=', 'En cours')->where('vu_chef_division', '=', '0')/* ->orwhere('motif', '=', 'Rejetée pour incorformité au niveau des montants') */->where('vu_verifi', '=', 1)->where('back_verifi', '=', 0)->where('date_decision', '=', null)->get());
        $dmd_back = count(demande::where('back_chef_division', '=', 1)->get());
