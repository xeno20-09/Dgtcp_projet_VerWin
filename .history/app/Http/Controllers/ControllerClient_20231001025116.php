...
class ControllerClient extends Controller
{
    ...
    public function Fclient(Request $request, $id)
    {
        ...
        $demande = demande::where('numero_doss', '=', $num_doss)
            ...
            ->get();
        $dema = demande::where('numero_doss', '=', $num_doss)
            ->where('date', '=', $date_depot)
            ->first();
        $test = count($demande);
        $nom_client = $dema->nom_client;

        $prenom_client = $dema->prenom_client;
        ...
        $demandes = demande::where('numero_doss', '=', $num_doss)
            ...
            ->first();
        $pic = 0;
        if ($prenom_client != $prenom && $nom_client != $nom) {
        ...
        if ($test == 0) {
            $message = 'La demande n\'existe pas désolé';
            return view('client.demande_client_i', compact('user', 'message'));
        }
    }
    // Generate PDF
