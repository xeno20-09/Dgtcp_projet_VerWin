...
class ControllerVerificateur extends Controller
{
    ...
    public function rejet(Request $request, $idm, $libellePiece, $referencePiece, $datePiece, $date, $restant, $montantligne, $montantdmd,$montantinitial)
    {
        $id = Auth::id();
        $user = User::find($id);
        ...
        foreach ($libellePiece as $key => $libellepiece) {
            $themontant += $montantligne[$key];
        }

        // Parcourez toutes les pièces soumises
        for ($key = 0; $key <= count($libellePiece) - 1; $key++) {
            $dmd_pieces = new Piece();
            $dmd_pieces->id_dmd = $idm;
            $dmd_pieces->libellepiece = $libellePiece[$key];
            $dmd_pieces->referencespiece = $referencePiece[$key];

            if (isset($montantligne[$key])) {
                $dmd_pieces->montantligne = $montantligne[$key];
                $dmd_pieces->montantrestant = $restant[$key];
                $dmd_pieces->montantinitial = $montantinitial[$key];
            } else {
            ...
            $lastPiece = Piece::where('referencespiece', $referencePiece[$key])->first();
