...
class ControllerSecretaire extends Controller
{
    ...
    public function store_search_ask_suite(Request $request, $id)
    {
        ...
        if ($dmd_suite == null) {
            ...
            $derniereValeur = $dernierNumeroDossier->numero_doss;
                $dernierCaractere = substr($derniereValeur, -1);
                $demande = demande::where('numero_doss', '=',  $dernierCaractere)->get();
                $piece = demande::where('numero_doss', '=',  $dernierCaractere)->get();

            
            return view('secretaire.recap', compact('montantr', 'numeroDossier', 'demande', 'user', 'le_n_dmd_c', 'le_n_dmd_v', 'le_n_dmd_e', 'le_n_dmd_s', 'dmd_n_lu'));
        } else {
