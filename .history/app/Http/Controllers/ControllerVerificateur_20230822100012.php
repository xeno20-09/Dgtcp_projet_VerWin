    ...
    public function store(Request $request, $idc)
    {
        ...
        $jointure = DB::table('users')
            ...
            if ($request->hasFile('file')) {
                ...
                $idc = $request->input('idc'); // Assurez-vous d'avoir l'ID correct depuis le formulaire
        
                $demande = Demande::find($idc);
                $date = now();
        
                return view('verificateur.infopiece', compact('numberinput', 'fileNames', 'date', 'demande'));
            } else {
