...
class ControllerVerificateur extends Controller
{
    ...
    public function checkref(Request $request, $idm)
    {
        ...
        for ($i = 0; $i < count($referencesPieces); $i++) {
            $dmd_verificateur = piece::find($referencesPieces[$i]);
            dd($dmd_verificateur);
            if (($dmd_verificateur->montantrestant != 0)){
                  if (($dmd_verificateur->montantrestant != 0) && ($dmd_verificateur->montanligne != 0)) {
                $bon++;
            }
            if (($dmd_verificateur->montantrestant == null) && ($dmd_verificateur->montanligne != 0)) {
                $bad++;
            }
            }
        ...
        if (($bon == $demande->nombre_doss) || ($dmd_verificateur == null)) {
            return view('verificateur.infopiece', compact('dmd_back', 'date', 'demande', 'dmd_n_lu', 'user'));
        } else {
