...
class ControllerClient extends Controller
{
    ...
    public function Fclient(Request $request, $id)
    {
        ...
        if($check_type_prs = demande::where('nom_client', '=', $nom)    ->where('prenom_client', '=', $prenom)    ->first()
        ) {
            $type_prs=$check_type_prs->type_prs;
        }
        elseif($check_type_prs = demande::where('nomsociete', '=', $nom)            ->where('nomsociete', '=', $prenom)->first()
        ) {
            $type_prs=$check_type_prs->type_prs;
