...
class ControllerVerificateur extends Controller
{
    ...
    public function store(Request $request, $idc)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            $demande = demande::where('id', '=', $idc)->first();
            $file = $fileNames[0];
            $numdoss = $demande->numero_doss;

            $date = now();
            $ref = $date . substr($numdoss, 0, 2);
            return view('verificateur.infopiece', compact('numberinput', 'ref', 'file', 'date', 'demande', 'dmd_n_lu', 'user', 'jointure'));
        } else {
