...
class ControllerVerificateur extends Controller
{
    ...
    public function store(Request $request, $idc)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            $change = Piece::where('id_dmd', $idc)->orderByDesc('created_at')->first();
            $chan = Piece::where('id_dmd', $idc)->orderByDesc('created_at')->first();

            $chan->montantrestant = $change->montantrestant - $montant;
            $change->update();
