...
class ControllerVerificateur extends Controller
{
    ...
    public function  form(Request $request, $id_dmd)
    {
        ...
        $dmd_back = count(demande::where('back_verifi', '=', 1)->get());
        $piece = piece::where('id_dmd', '=', $id_dmd)->where('vu_verif', '=', 1)->get();

        $test = piece::where('id_dmd', '=', $id_dmd)->where('vu_verif', '=', 0)->get();
    ...
    public function liste(Request $request)
    {
        ...
        $demande = demande::where('id_verifi', '=', $id)->where('vu_verifi', '=', 1)->where('vu_secret', '=', 1)->get();
        $le_n_dmd = count($demande);
        $dmd_n_lu = count(demande::/* where('id_verifi', '=', $id)-> */where('vu_verifi', '=', 0)->where('vu_secret', '=', 1)->get());
    ...
    public function rejet(Request $request, $idm 'libellePiece' => $libellePiece[$i],
    'referencePiece' => $referencePiece[$i],
    'datePiece' => $datePiece[$i],
    'date' => $date,
    'restant' => $restant,
    'montantligne' => $montantligne[$i],
    'montantdmd' => $montantdmd,)
    {
        $id = Auth::id();
        $user = User::find($id);
        ...
        // Parcourez toutes les pièces soumises
        foreach ($libellePiece[$i] as $key => $libellepiece) {
            $themontant += $montantligne[$i][$key];
        }

        // Parcourez toutes les pièces soumises
        for ($key = 0; $key <= count($libellePiece[$i]) - 1; $key++) {
            $dmd_pieces = new Piece();
            $dmd_pieces->id_dmd = $idm;
            $dmd_pieces->libellepiece = $libellePiece[$i][$key];
            $dmd_pieces->referencespiece = $referencePiece[$i][$key];

            if (isset($montantligne[$i][$key])) {
                $dmd_pieces->montantligne = $montantligne[$i][$key];
                $dmd_pieces->montantrestant = $restant[$key];
                $dmd_pieces->montantinitial =$montantinitial[$i][$key];
            } else {
                $dmd_pieces->montantligne = 'NEANT';
            }

            $dmd_pieces->date =  $date[$key];

            if (isset($datePiece[$i][$key])) {
                $dmd_pieces->dateexpi =$datePiece[$i][$key];
            }

            $n_verificateur = $l_verificateur . " " . $f_verificateur;
            $dmd_pieces->nom_v = $n_verificateur;

            $lastPiece = Piece::where('referencespiece', $referencePiece[$i][$key])->first();
