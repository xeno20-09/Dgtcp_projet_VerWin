...
class ControllerSecretaire extends Controller
{
    ...
    public function store_form_ask(Request $request)
    {
        ...
        $n = substr($dernierNumeroDossier, 7, 6);
        if ($dernierNumeroDossier) {
            $derniereValeur = $dernierNumeroDossier;

            if (str_contains($derniereValeur, 'DTr')) {

                $segments[] = explode('DTr', $derniereValeur);
        ...
        $numeroDossier = "DTr" . "-" . sprintf('%06d', $new) . "-" . $anneeDemande;
