...
class ControllerDivision extends Controller
{
    ...
    public function liste_demandes_n(Request $request)
    {
        $id = Auth::id();
        $user = User::where('id', '=', $id)->get();
        $demande = demande::where('status_dmd', '=', 'En cours')->where('vu_verifi', '=', 1)->where('back_verifi', '=', 0)->where('date_decision', '=', null)->get();
    ...
    public function retour(Request $request, $idc)
    {
        ...
        return view('chef_division.liste_demande_n', compact('demande', 'dmd_back', 'dmd_n_lu', 'user', 'jointure', 'jointure1'));
    }
