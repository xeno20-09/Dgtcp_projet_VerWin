...
$found=0;
$notfound=0;
            for ($i = count($data['referencespiece'][$key]) - 1; $i >= 0; $i--) {
                $dmd_verificateur = piece::where('referencespiece', '=',$data['referencespiece'][$key])
                    //->latest('id') // Triez par ordre décroissant de la colonne 'id'
                    ->first(); // Obtenez le premier résultat, qui est le dernier enregistrement
                if ($dmd_verificateur) {
                    $found++;
...
dd(count($data['referencespiece'][$key]));
if($found==count($data['referencespiece'][$key]))
{
