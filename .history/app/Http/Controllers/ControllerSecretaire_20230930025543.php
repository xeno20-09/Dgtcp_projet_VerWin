...
Alternative convertion*/
        ...
        $numero = $phoneNumberUtil->parse($numeroTelephone, null);

        // Récupérez le code de pays (par exemple, "US" pour les États-Unis)
        $codePays = $phoneNumberUtil->getRegionCodeForNumber($numero);

        // Utilisez libphonenumber pour obtenir le nom du pays
        $locale = 'fr_FR'; // Définissez la locale souhaitée pour le nom du pays
        $nomPays = $phoneNumberUtil->getRegionDisplayName($codePays, $locale);

        dd($request->input('codecountry'));
