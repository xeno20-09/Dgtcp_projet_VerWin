...
*/
        ...
        $url = curl_init('http://api.exchangerate.host/historical?access_key=' . $access_key . '&date=' . $date . '&source=' . 'XOF' . '&format=' . '1');
        // get the (still encoded) JSON data:
        $aa = curl_exec($url);
        curl_close($url);
        $r = json_decode($aa, true);
        $quotes = $r['quotes'];
        if (array_key_exists('XOFEUR', $quotes)) {
            $y = $quotes['XOFEUR'];
            dd($y);
        } else {
            // La clé 'XOFEUR' n'existe pas dans le tableau des taux de change
            // Vous pouvez gérer cette situation comme vous le souhaitez
            // Par exemple, afficher un message d'erreur ou une valeur par défaut
        }
        $json = curl_exec($ch);
        curl_close($ch);
