...
class ControllerVerificateur extends Controller
{
    ...
    public function checkref(Request $request, $idm)
    {
        ...
        foreach ($referencesPieces as $reference) {
            $dmd_verificateur = piece::where('referencespiece', '=', $reference)
                ->orderBy('montantrestant') // Triez par ordre décroissant de la colonne 'id'
                ->latest()->first(); // Obtenez le premier résultat, qui est le dernier enregistrement
    ...
    public function store_piece_dmd(Request $request, $id_d)
    {
        ...
        foreach ($data['libellepiece'] as $key => $libellePiece) {
            ...
            $lastPiece = Piece::where('referencespiece',  $dmd_pieces->referencespiece)->first();
            ...
            if ($lastPiece) {
                ...
                $dmd_pieces->montantrestant = $dmd_pieces->montantligne - $lastPiece_s->montantrestant;
            } else {
                $dmd_pieces->montantrestant =  $dmd_pieces->montantligne - $dmd_verificateur->montant;
                $dmd_pieces->montantinitial = $dmd_verificateur->montant;
            }

            // dd($lastPiece->montantrestant);
    ...
    public function store(Request $request, $idc)
    {
        ...
        //  dd($dmd_verificateur->nombre_doc);
