    ...
    public function checkref($libellePiece, $referencePiece, $datePiece, $montantligne,  $dateExpi, $idm)
    {
       ...
       $montant = demande::where('id', '=', $idm)->first();
        ...
        if(count($referencePiece)==$found){
            $montant = demande::where('id', '=', $idm)->first();
             $montantdmd = $montant->montant;
             $montantrestant = array_sum($montantligne) - $dmd_verificateur->montantrestant;         
             }
        //
        $id = Auth::id();
        $user = User::where('id', '=', $id)->get();
        ...
        $dmd_back = count(demande::where('back_verifi', '=', 1)->get());
        return view('verificateur.infopiece', compact('montantligne', 'restant', 'montantrestant', 'found', 'notfound', 'libellePiece', 'datePiece', 'montantdmd', 'referencePiece', 'dmd_back', 'demande', 'dmd_n_lu', 'user'));
    }
