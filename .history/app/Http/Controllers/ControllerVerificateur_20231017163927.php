...
class ControllerVerificateur extends Controller
{
    ...
    public function checkref($libellePiece, $referencePiece, $datePiece, $montantligne,  $dateExpi, $idm)
    {
        ...
        //        dd(array_sum($montantligne));

        for ($i = count($referencesPieces); $i > 0; $i--) {
        ...
        for ($i = $demand->nombre_doc; $i > 0; $i--) {
            ...
            $dmd_pieces->save();
            if ($i == 1) {
                $dmd_piece = piece::find($idm);

                $dmd_piece->montantrestant = array_sum($montantligne) - $montantdmd;
                $dmd_piece->update();
            }
        }
