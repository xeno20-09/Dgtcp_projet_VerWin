...
class ControllerAdminSGBD extends Controller
{
    ...
    /**
     * Display the client's etat des deman page.
     *
     * @return \Illuminate\View\View
     */
    ...
    public function getdevis()
    {
        ...
        return view('Admin_SGBD.etatdevise', compact('codeToDevise', 'groupedDemandes', 'categories', 'user', 'le_n_dmd_c', 'dmd_back', 'le_n_dmd_v', 'le_n_dmd_e', 'le_n_dmd_s', 'dmd_n_lu', 'admin'));
    }
    ...
    public function getsociete()
    {
        ...
        $group = demandes::select('nomsociete', 'devise', DB::raw('SUM(montant) as total'))
            ->groupBy('nomsociete', 'devise')
            //->orderBy('devise', 'desc')
            ->get();
        $nbrs = count($group);

        return view('Admin_SGBD.etatsociete', compact('nbrs', 'group', 'devise', 'user', 'le_n_dmd_c', 'dmd_back', 'le_n_dmd_v', 'le_n_dmd_e', 'le_n_dmd_s', 'dmd_n_lu', 'admin'));
    }
    ...
    public function modify_user(Request $request, $id)
    {
        ...
        $user = User::where('id', '=', $id)->get();
        return view('Admin_SGBD.form_user', compact('user', 'admin'));
    }
    ...
    public function listedmd(Request $request)
    {
        ...
        if ($status != 'All') {
            ...
            return view('Admin_SGBD.liste', compact('demande', 'sdate', 'fdate', 'status', 'number', 'user', 'jointure', 'jointure1', 'jointure2', 'jointure3', 'admin'));
        } else {
            // Utilisez la méthode whereNotNull pour vous assurer que les champs ne sont pas vides
            $demande = demande::whereBetween('date', [$fdate, $sdate])->get();
            $id = Auth::id();
    ...
    public function laliste(Request $request)
    {
        ...
        if ($status != 'All') {
            ...
            $number = count($demande);
            //dd($demande);
            return view('Admin_SGBD.laliste', compact('demande', 'sdate', 'devise', 'fdate', 'status', 'number', 'user', 'jointure', 'jointure1', 'jointure2', 'jointure3', 'admin'));
        } else {
            // Utilisez la méthode whereNotNull pour vous assurer que les champs ne sont pas vides
    ...
    public function detailles(Request $request, $id)
    {
        ...
        $jointure3 = DB::table('users')
            ...
            ->get();
        $dmd_n_lu = count(
            demande::where('vu_damf', '=', 0)
                ->where('vu_chef_bureau', '=', 1)
                ->get(),
        );
        $piece = piece::where('id_dmd', '=', $id)->first();

        return view('Admin_SGBD.detaille_demande', compact('demande', 'sdate', 'devise', 'fdate', 'piece', 'user', 'dmd_n_lu', 'jointure', 'jointure1', 'jointure2', 'jointure3', 'admin'));
    }
    ...
    public function createPDF()
    {
        ...
        return $pdf->download('listeUser.pdf', 'admin');
    }
    ...
    public function listePDF($status, $sdate, $fdate)
    {
        ...
        if ($status != 'All') {
            ...
            $pdf = PDF::loadView('Admin_SGBD.pdf_view1', $demande = ['numero_doss', 'date', 'montant', 'devise', 'montant_con', 'staus_dmd', 'admin']);
    ...
    public function lalistePDF($status, $sdate, $fdate, $devise)
    {
        ...
        if ($status != 'All') {
            ...
            return $pdf->download('listedemande.pdf', 'admin');
        } else {
            $demande = demande::whereBetween('date', [$fdate, $sdate])
                ->where('devise', $devise)
                ->get();
    ...
    public function lalisteetatsPDF($test)
    {
        ...
        if ($test == 'Devise') {
            ...
            return $pdf->download('listeetats.pdf', 'admin');
        }
