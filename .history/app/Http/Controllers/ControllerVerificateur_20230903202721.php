...
$restant = $dmd_verificateur->montantrestant;
            ...
            if (($bon == 0) && ($bad == 0)) {
                ...
                $e = 'non';
                $restant = $dmd_verificateur->montantrestant;
                return view('verificateur.infopiece', compact('montantdmd'$restant = $dmd_verificateur->montantrestant;, 'referencesPieces', 'e', 'dmd_back', 'date', 'demande', 'dmd_n_lu', 'user'));
            } else {
    ...
    public function store_piece_dmd(Request $request, $id_d)
    {
        ...
        if (isset($data['libellepiece'])) {
            ...
            for ($i = 0; $i < count($libellePieces); $i++) {
                ...
                $checkexist = Piece::where('referencespiece', $referencesPieces[$i])->where('referencespiece', $referencesPieces[$i])->orderByDesc('created_at')->first();
                ...
                $dernier_piece_enregistre = Piece::where('referencespiece', $referencesPieces[$i])->orderByDesc('created_at')->first();
                ...
                if ($i == 0) {
                    ...
                    if ($dernier_piece_enregistre) {
                        $dmd_pieces->montantrestant =   $dernier_piece_enregistre->montantrestant -  $montantdmd;
                    } else {
                        $dmd_pieces->montantrestant =  $montantLignes[$i] - $dernier_piece_enregistre->montantinitial;
                    }
                    /* } */
                }
