...
*/
        ...
        unset($u[30]);
        unset($u[36]);
        $unwantedKeys = [21, 25, 30, 37, 132, 153, 159, 160, 161, 162, 163, 166];
        ...
        foreach ($u as $key => $value) {
            if (!in_array($key, $unwantedKeys)) {
                ...
                $new[] = $casse;
            }        foreach ($codeToDevise as $key1 => $value) {

            if($new[$key]==$codeToDevise[$key1]){
                $pays[]=$codeToDevise[$key1];
            }}
        }

      

        $numbercode = count($new);
