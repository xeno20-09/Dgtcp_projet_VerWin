...
class ControllerDivision extends Controller
{
    ...
    public function liste_demandes_n(Request $request)
    {
        $id = Auth::id();
        $user = User::where('id', '=', $id)->get();
        $demande = demande::where('status_dmd', '=', 'En cours')->where('id_chef_divisio', '=', '0')->orwhere('motif', '=', 'Rejetée pour incorformité au niveau des montants')->where('vu_verifi', '=', 1)->where('back_verifi', '=', 0)->where('date_decision', '=', null)->get();
