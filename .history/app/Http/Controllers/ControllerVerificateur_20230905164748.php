...
class ControllerVerificateur extends Controller
{
    ...
    public function checkref($libellePiece, $referencePiece, $datePiece,$montantlign $idm)
    {
        // dd($libellePiece);
        ...
        for ($i = count($referencesPieces) - 1; $i >= 0; $i--) {
            ...
            if (($bon == $demand->nombre_doc)) {
                ...
                return view('verificateur.infopiece', compact('libellePiece', 'referencePiece', 'datePiece', 'montantdmd', 'restant', 'referencesPieces', 'e', 'dmd_back', 'date', 'demande', 'dmd_n_lu', 'user'));
            }
            if (($bon == 0) && ($bad == 0)) {
                $restant = null;
                $e = 'non';
        ...
        if ($dmd_verificateur == null) {
            return view('verificateur.infopiece', compact('libellePiece', 'referencePiece', 'datePiece', 'montantdmd', 'referencesPieces', 'dmd_back', 'date', 'demande', 'dmd_n_lu', 'user'));
        }
    }
    ...
    public function store(Request $request, $idc)
    {
        ...
        $resultat = $this->checkref(
            ...
            $datePiece = $data['exp_pieces'], // $datePiece
            $idc // $idm
        );
        return $resultat;
    }
