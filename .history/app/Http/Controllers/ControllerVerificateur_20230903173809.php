...
class ControllerVerificateur extends Controller
{
    ...
    public function checkref(Request $request, $idm)
    {
        ...
        for ($i = 0; $i < count($referencesPieces); $i++) {
            ...
            if (($bon == $demand->nombre_doc)) {
                ...
                return view('verificateur.infopiece', compact('montandmd','restant', 'referencesPieces', 'e', 'dmd_back', 'date', 'demande', 'dmd_n_lu', 'user'));
            }
            if (($bon == 0) && ($bad == 0)) {
                $restant = null;
                $e = 'non';
                return view('verificateur.infopiece', compact('restant', 'referencesPieces', 'e', 'dmd_back', 'date', 'demande', 'dmd_n_lu', 'user'));
            } else {
        ...
        if ($dmd_verificateur == null) {
            return view('verificateur.infopiece', compact('referencesPieces', 'dmd_back', 'date', 'demande', 'dmd_n_lu', 'user'));
        }
    }
    ...
    public function store_piece_dmd(Request $request, $id_d)
    {
        ...
        return view('verificateur.recap_pieces', compact('montantdmd', 'demande', 'dmd_back', 'dmd_n_lu', 'user'));
    }
