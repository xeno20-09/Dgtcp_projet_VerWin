 ...
 */
    ...
    public function lalisteetatsPDF($test)
    {
        ...
        if ($test == 'Pays') {
            ...
            $liste = listedevise::all();
            $devis = demande::all();
            $grouped = demandes::select('nationalite','montant','', DB::raw('SUM(montant) as total'))
                ->groupBy('nationalite')
                ->get();
            view()->share([
                'grouped' => $grouped,
            ...
            $pdf = PDF::loadView('Admin_SGBD.pdf_view2', $grouped = ['devise', 'montant', 'nationalite', 'total']);
            return $pdf->download('listeetats.pdf');
        }
        ...
        if ($test == 'Société') {
            ...
            $group = demandes::select('nomsociete', DB::raw('SUM(montant) as total'))
                ->groupBy('nomsociete')
                ->get();
            $pdf = PDF::loadView('Admin_SGBD.pdf_view2', $test, $devise = ['devise', 'nationalite', 'nomsociete', 'total']);
            return $pdf->download('listeetats.pdf');
        }
    }
}
