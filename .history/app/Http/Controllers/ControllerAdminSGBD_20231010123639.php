 ...
 */
    ...
    public function listedmd(Request $request)
    {
        ...
        if ($status != 'All') {
            ...
            return view('Admin_SGBD.liste', compact('demande', 'sdate', 'devise', 'fdate', 'status', 'number', 'user', 'jointure', 'jointure1', 'jointure2', 'jointure3'));
        } else {
    ...
    public function listePDF($status, $sdate, $devise, $fdate)
    {
        /*         // retreive all records from db
        $data = user::all();
        // share data to view
        ...
        if ($status != 'All') {
            ...
            $pdf = PDF::loadView('Admin_SGBD.pdf_view', $demande = ['numero_doss', 'date', 'montant', 'devise', 'montant_con', 'staus_dmd']);
