...
class ControllerVerificateur extends Controller
{
    ...
    public function store_piece_dmd($libellePiece, $referencePiece, $datePiece, $id_d)
    {
        ...
        // Parcourez toutes les pièces soumises
        foreach ( as $key => $libellePiece) {
            $dmd_pieces = new Piece();
            $dmd_pieces->id_dmd = $id_d;
            $dmd_pieces->libellepiece = $libellePiece;
            $dmd_pieces->referencespiece = $da[$key];
            $dmd_pieces->date [$key];
            $dmd_pieces->montantligne = [$key];
    ...
    public function store(Request $request, $idc)
    {
        ...
        foreach ($data['pieces_doss'] as $key => $pieces_doss) {
            ...
            $dmd_verificateur->pieces = strval($data['pieces_doss'][$key]);
            $dmd_verificateur->refpiece = strval($data['ref_doss'][$key]);

            $dmd_verificateur->dateexpi = strval($data['exp_pieces'][$key]);
            //dd($dmd_verificateur);
            $this->store_piece_dmd($data['pieces_doss'][$key], $data['ref_doss'][$key], $data['exp_pieces'][$key]);
        }
        // dd(count($data['pieces_doss']));
