 ...
 */
    ...
    public function lalisteetatsPDF($test)
    {
      ...
      if($test=='Devise'){
       $liste = listedevise::all();
    $devise = demandes::select('devise')
        ->groupBy('devise', DB::raw('SUM(montant) as total'))
        ->get();   
        view()->share('liste', $devise);
        $pdf = PDF::loadView('Admin_SGBD.pdf_view2', $devise = ['devise','total']);
    ...
    $grouped = demandes::select('nationalite', DB::raw('SUM(montant) as total'))
        ->groupBy('nationalite')
        ->get();
        $pdf = PDF::loadView('Admin_SGBD.pdf_view2', $devise = ['nati','total']);
        return $pdf->download('listeetats.pdf'); 
      }
