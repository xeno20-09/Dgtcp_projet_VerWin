...
class ControllerSecretaire extends Controller
{
    ...
    public function home(Request $request)
    {
        ...
        $date = now();
        $dmd_back = count(demande::where('back_secret', '=', 1)->get());
    ...
    public function   get_list_details_ask(Request $request, $id)
    {
        ...
        return view('secretaire.detaille_demande', compact('demande', 'dmd_back', 'user', 'dmd_n_lu'));
    }
    /************************************Voir les details d'une demande************************************************ */
    ...
    public function store_update_form_ask(Request $request, $id_dmd)
    {
        ...
        // Redirection vers la page de liste des produits  
        $demande = demande::where('id_secret', '=', $id)->get();
        $le_n_dmd = count($demande);      $dmd_back = count(demande::where('back_secret', '=', 1)->get());
        return view('secretaire.liste_demande', compact('demande', 'dmd_back', 'dmd_n_lu', 'user'));
    ...
    public function get_search_ask_suite(Request $request)
    {
        ...
        $user = User::where('id', '=', $id)->get();
        $date = now();
        $dmd_n_lu = count(demande::where('reponse_damf', '=', 1)->get());      $dmd_back = count(demande::where('back_secret', '=', 1)->get());
    ...
    public function store_search_ask_suite(Request $request, $id)
    {
        ...
        if ($dmd_suite == null) {
            ...
            return view('secretaire.recap', compact('montantr', 'numeroDossier', 'demande', 'user', 'le_n_dmd_c', 'le_n_dmd_v', 'le_n_dmd_e', 'le_n_dmd_s', 'dmd_n_lu', 'piece'));
        } else {
            ...
            $le_n_dmd_e = count($demande_echec);
            $le_n_dmd_s = count($demande_suspendre);
            $dmd_n_lu = count(demande::where('vu_chef_bureau', '=', 0)->get());      $dmd_back = count(demande::where('back_secret', '=', 1)->get());
            // dd($numeroDossier);
