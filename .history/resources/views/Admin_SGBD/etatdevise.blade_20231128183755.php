...
<script>
  ...
  function drawChart() {
    var data = google.visualization.arrayToDataTable([
      ['Devises','Devise', 'Montant Total'], // Deux colonnes : Devise et Montant Total
      @for ($i = 0; $i < count($categories); $i++)
    @foreach($codeToDevise as $clef => $valeur)
        @if ($categories[$i]->devise == $valeur)
            @php $categories[$i]->devise = $clef; @endphp
        @endif
    @endforeach
    ['{{ $categories[$i]->devise }}',{{ $categories[$i]->devise }}, {{ $categories[$i]->total }}],
@endfor
