...
<script>
    $(document).ready(function() {
        $('#currency_from_p').on('change', function() {
            ...
            var tok=document.querySelector('meta[name="csrf-token"]').attr('content')           if (monnaie !== 'null') {
                $.ajax({
    type: 'POST',
    url: '/get-devis',
    data: {
        monnaie: monnaie,
        token: $('meta[name="csrf-token"]').attr('content') // Récupère le jeton CSRF à partir de la balise meta
    },
