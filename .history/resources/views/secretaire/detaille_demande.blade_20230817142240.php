            ...
            <div style="max-width: fit-content;" class="card mb-3 d-flex flex-row">
                <ul class="list-group list-group-flush flex-column gap-3">
                    ...
                    <li class="list-group-item">Pieces joint:
                        @if (Str::endsWith($item['file'], ['.pdf', '.jpeg', '.png', '.gif', '.bmp']))
                            <img src="{{ asset('files/' . $item['file']) }}" alt="Aperçu du fichier" width="100">
                        @else
                            <a href="{{ asset('files/' . $item['file']) }}" target="_blank">Voir le fichier</a>
                        @endif
                    </li>
