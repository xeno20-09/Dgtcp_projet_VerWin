    ...
    <div class="row">
        ...
        <div class="col">
            <div class="form-group">
                ...
                <input name="montant_in" type="number" onchange="check()" class="form-control" id="" placeholder="Montant ">
            </div>
        </div>
        ...
        <div class="col" id="valeur_p">
          ...
          {{--       @foreach ($devise as $devise)
                    @if($devise->devise=='Dollar canadien')
                    {{$devise->valeur}}
                    @else
                    0
                    @endif
...
function check() {
            ...
            var mon = document.getElementById('currency_from_p').value ;
            var val = document.getElementById('valeur_p').value;
            var mont=document.getElementById('mont_fcfa_p');
            console.log(mon);
            console.log(val);

            
            var devises = @json($dev);
            for (const de of devises) {
                if (de.devise == 'Euro') {
                    // La devise sélectionnée correspond à celle de $devise->devise
                    // Vous pouvez effectuer ici l'action souhaitée
                    console.log('Valeur de la devise : ' + de.valeur);
                    break; // Sortir de la boucle car la correspondance a été trouvée
                }
            }
}
check();
...
function valeur_p() {
        ...
        // Parcourir les devises et effectuer la comparaison
