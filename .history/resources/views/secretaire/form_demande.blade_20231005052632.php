    ...
    <div class="row">
        ...
        <div class="col" id="valeur_p">
            <div class="form-group">
                ...
                @foreach ($listedevis as $name)
                @php
                    $found = false; // Initialisez une variable pour vérifier si la devise a été trouvée
                @endphp
            
                @foreach ($dev as $devise)
                    @if ($devise->devise == $name->nom)
                        {{ $devise->valeur }}
                        @php
                            $found = true; // Marquez la devise comme trouvée
                            break; // Sortez de la boucle interne une fois qu'elle est trouvée
                        @endphp
                    @endif
                @endforeach
            
                @if (!$found)
                    0 <!-- Si la devise n'est pas trouvée, affichez 0 -->
                @endif
            @endforeach
...
<script>
var cur=document.getElementById('currency_from_p');
var cur=document.getElementById('valeur_p');

           // Lorsque l'élément avec l'ID 'nom' perd le focus (lorsque l'utilisateur a terminé de saisir le nom)
           $().on('blur', function() {
        ...
        if (monnaie !== '') {
            // Effectuer une requête AJAX vers l'URL '/get-age' définie dans votre route Laravel
            $.ajax({
                type: 'POST', // Utiliser une requête POST pour envoyer des données au serveur
                url: '/get-devis', // L'URL vers laquelle la requête est envoyée (doit correspondre à votre route Laravel)
                data: { devise: monnaie }, // Les données que vous envoyez au serveur, dans ce cas, le nom saisi
                success: function(data) {
                    // La fonction à exécuter si la requête réussit
                    // 'data' contient la réponse JSON renvoyée par le serveur

                    // Mettre à jour le champ 'age' avec l'âge récupéré dans la réponse JSON
                    $().val(data.valeur);
                }
            });
        }
