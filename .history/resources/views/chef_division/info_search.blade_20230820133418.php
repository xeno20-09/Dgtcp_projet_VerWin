...
@section('content')
    @foreach ($info as $row1)
        @if ($row1->count() == 0)
            <div class="alert alert-danger">
                <strong>Votre recherche a {{ "request()->input('query')" }} retournée {{ $row1->count() }}
                    resultat</strong>
    ...
    @else
        <div class="alert alert-success">
            <strong>Votre recherche a "{{ $query }}" retournée {{ $row1->count() }} resultat(s) </strong>
            <div class="progress-bar"></div>
        </div>
        ...
        <div style="max-width: fit-content;" class="card mb-3 d-flex flex-row">
            <ul class="list-group list-group-flush flex-column gap-3">
                ...
                <li class="list-group-item">Devise: {{ $row1->devise_in }}</li>
