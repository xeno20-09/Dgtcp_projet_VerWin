...
class ControllerClient extends Controller
{
    ...
    public function Fclient(Request $request, $id)
    {
        ...
        $nom_clt = $data['nom_client'];
        $prenom_client = $data['prenom_client'];
        $num_compt_client = $data['num_compt_client'];
        $num_compt_benefi = $data['num_compt_benefi'];
        $date_depot = $data['date_depot'];

        $demande = demande::where('numero_doss', '=', $num_doss)
            ->where('nom_client', '=', $nom_clt)
            ->where('prenom_client', '=', $prenom_client)
            ->where('num_compt_client', '=', $num_compt_client)
            ->where('num_compt_benefi', '=', $num_compt_benefi)
            ->where('date', '=', $date_depot)
            ->get();
        $test = count($demande);
        if ($test != 0) {
            return view('client.demande_client_t', compact('user', 'demande'));
        }
        if ($test == 0) {
